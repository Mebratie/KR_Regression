...
#################################################    
    ...
    solutions = [
        [solve_for_lambda(combined_dataset, c, d, l, m) for l in lvals]
    ...
    f_alpha_expressions = [
        process_dataset(combined_dataset, lvals, solutions_combined) for combined_dataset in combined_datasets
    ]
    y_values_dicts_list = solutions_combined

    ndim = num_variables
    nlvals = len(lvals)
#     f_vector = np.zeros((nsplits, nlvals, len(combined_datasets[-1])))
